...
if not GROQ_API_KEY:
  st.error(
      "⚠️ GROQ_API_KEY Streamlit Secrets bölməsində tapılmadı! Lütfən"
...
if st.session_state.guest_plan != st.session_state.prev_plan:
  if st.session_state.guest_plan == "Pro":
    st.toast(
        "💎 PRO ELITE MODE AKTİVLAŞDIRILDI! Neyron sistemlər maksimum gücə"
        " keçdi! 🚀",
        icon="⚡",
    )
  st.session_state.prev_plan = st.session_state.guest_plan
...
if not st.session_state.onboarding_done:
  ...
  def show_onboarding():
    ...
    st.markdown(
        "💬 **Chat & Search Box:** Type your questions, code queries, or"
        " commands directly."
    )
    st.markdown(
        "⚡ **Daily Limit:** You have a limit of 50 questions per day! (Gündəlik"
        " 50 sual limitiniz var)"
    )
    st.markdown("🌐 **Language Selection (Sidebar):** Switch app language anytime.")
    st.markdown(
        "⚙️ **Settings & Personas (Sidebar):** Adjust creativity and select"
        " personas like 👑 Məntiq Kralı."
    )
...
translations = {
    "Azərbaycan": {
        ...
        "title_pro": "AliGo PRO - Süni İntellekt və Media Mərkəzi",
    ...
    "English": {
        ...
        "title_pro": "AliGo PRO - AI & Media Hub",
    ...
    "Русский": {
        ...
        "title_pro": "AliGo PRO - Центр ИИ и Медиа",
...
try:
  if hasattr(st, "experimental_user") and getattr(
    ...
    user_name = getattr(st.experimental_user, "name", None) or getattr(
...
if (
    limit_data_initial["remaining"] <= 0
    and "limit_alert_shown" not in st.session_state
):
  ...
  def limit_lock_dialog():
    ...
    st.info(
        f"Lütfən 12 saat sonra, {reset_dt.strftime('%d.%m.%Y %H:%M')} tarixində"
        " yenidən cəhd edin."
    )
...
def ask_groq_ai(messages_history, user_plan="Flash"):
  ...
  base_identity = (
      ...
      "1. SALAMLAŞMA QAYDASI: Yalnız və yalnız söhbətin ƏN İLK mesajında"
      " (tarixçə boş olanda və ya ilk dəfə yazanda) nəzakətlə salam ver və"
      " özünü AliGo olaraq təqdim et. Əgər bu, davam edən söhbətdirsə (yəni"
      " əvvəldən mesajlar varsa), heç vaxt təzədən 'Salam, mən AliGo-yam' deyib"
      " özünü tanıtma! Birbaşa və təmkinlə sualın cavabına keç.\n"
      "2. KİMLİK: Heç vaxt Google, OpenAI və ChatGPT olduğunu demə. Sən AliGo"
      " Süni İntellekt, Şəkil və Media Mərkəzisən!\n"
      "3. AĞILLI CAVABLAR: Hər zaman son dərəcə ağıllı, dərin məntiqli, dəqiq"
      " və peşəkar cavablar ver. Kod, məntiq və ya ümumi suallara ən üstün"
      " səviyyədə cavab hazırla.\n"
  )
  ...
  pro_boost = (
      " [PRO ELITE MODE: Ən dərin, qüsursuz, maksimum sürətli və premium"
      " səviyyədə cavab ver]"
      if user_plan == "Pro"
      else ""
  )
  system_instruction = (
      base_identity + persona_text + f"Aktiv rejim: {user_plan}." + pro_boost
  )
  ...
  trimmed_history = (
      messages_history[-5:] if len(messages_history) > 5 else messages_history
  )
...
with col_top1:
  ...
  st.markdown(
      f"<h4 style='color: #00f2fe; margin-top: 5px;'>{top_title_text}</h4>",
      unsafe_allow_html=True,
  )
...
if st.session_state.show_aliai:
  ...
  if st.session_state.trigger_prompt:
    ...
    if limit_data_check["remaining"] > 0:
      ...
      response = (
          "🛑 **Limitiniz bitdi!** Siz günlük 50 sual limitinizi"
    ...
    if message["role"] == "user":
      ...
      if "__IMAGE_URL__" in msg_content:
        ...
        if len(parts) > 1:
          ...
          st.markdown(
              f'<img src="{img_link}" style="width:100%; border-radius:15px;'
              " margin-top:10px; margin-bottom:10px; box-shadow: 0 4px 20px"
              ' rgba(0,242,254,0.3);" />',
              unsafe_allow_html=True,
          )
    ...
    if (
      ...
      if edit_action != "Seçim edin...":
        try:
          ...
          st.image(
              processed_img, caption=f"Redaktə olundu: {edit_action}", width=300
          )
